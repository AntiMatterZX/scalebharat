...
export default function EnhancedInvestorAnalyticsPage() {
    ...
    const getInvestorId = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (profile.userType === 'investor' && profile.investorProfile?.id) {
            setInvestorId(profile.investorProfile.id)
          }
        }
      } catch (error) {
  ...
  if (!investorId) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <p className="text-muted-foreground mb-4">Investor profile not found</p>
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="text-green-600 border-green-600">
              ...
              {realTimeMetrics.active_users_5min} active startups
            </Badge>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Deal Success Rate</CardTitle>
              <CardDescription>From matches to closed deals</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-primary mb-2">
                {totalMatches?.metric_value && closedDeals?.metric_value
                  ? ((closedDeals.metric_value / totalMatches.metric_value) * 100).toFixed(1)
                  : 0}%
              </div>
              <Progress 
                value={totalMatches?.metric_value && closedDeals?.metric_value
                  ? (closedDeals.metric_value / totalMatches.metric_value) * 100
                  : 0} 
                className="h-2" 
              />
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Interest Conversion</CardTitle>
              <CardDescription>Matches to interested deals</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-primary mb-2">
                {totalMatches?.metric_value && interestedMatches?.metric_value
                  ? ((interestedMatches.metric_value / totalMatches.metric_value) * 100).toFixed(1)
                  : 0}%
              </div>
              <Progress 
                value={totalMatches?.metric_value && interestedMatches?.metric_value
                  ? (interestedMatches.metric_value / totalMatches.metric_value) * 100
                  : 0} 
                className="h-2" 
              />
            </CardContent>
        ...
        <Tabs defaultValue="pipeline" className="space-y-6">
          ...
          <TabsContent value="pipeline" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Track your investment journey from discovery to deals</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {step.count} {step.count === 1 ? 'startup' : 'startups'}
                          </p>
                        </div>
                      </div>
          ...
          <TabsContent value="trends" className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  <CardTitle>Startup Discovery Trend</CardTitle>
                  <CardDescription>Daily startup views over the last 30 days</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Breakdown of investment activity</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {getMetricsByCategory('matching').map((metric) => (
                    ...
                    {getMetricsByCategory('deals').map((metric) => (
                      <div key={metric.metric_name} className="flex justify-between items-center">
                        <span className="capitalize">{metric.metric_name.replace('_', ' ')}</span>
                        <span className="font-bold">{metric.metric_value}</span>
                      </div>
          ...
          <TabsContent value="realtime" className="space-y-6">
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">New Matches Today</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">Meetings Today</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold text-orange-600">
                    {realTimeMetrics.meetings_today}
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Real-time Investment Activity</CardTitle>
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="text-center p-4 border rounded-lg">
                    <div className="text-lg font-bold">{realTimeMetrics.profile_views_today}</div>
                    <div className="text-sm text-muted-foreground">Profile Views Today</div>
                  </div>
                  <div className="text-center p-4 border rounded-lg">
